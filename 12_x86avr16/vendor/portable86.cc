...
uint16_t readmemw(uint32_t s, uint16_t a) {
    ...
    uint8_t l = machine_read(s + a);
    uint8_t h = machine_read(s + ((a+1) & 0xffff));
    return (h<<8) | l;
}
...
inline uint8_t fetch_byte() { return machine_read(seg_cs + (ip++)); }

// Считывание WORD из потока CS:IP
uint16_t fetch_word() {

    uint8_t l = fetch_byte();
    return ((uint16_t)fetch_byte() << 8) | l;
}
...
uint8_t geteab() {
    ...
    if (cpu_mod == 3)
        ...
        return machine_read(segment + eaaddr);
}
...
void ud(int type) {
    ...
    printf("[%04x:%04x] %02x UNDEFINED %02x\n", seg_cs, ip, machine_read(seg_cs + ip), type);
    // printf("fs=%04x\n", seg_fs);
...
void extended() {
    ...
    switch (opcode) {
        ...
        // JO, JNO
        case 0x80: offset = (int16_t)fetch_word(); if   (flags&V_FLAG)  ip += offset; break;
        case 0x81: offset = (int16_t)fetch_word(); if (!(flags&V_FLAG)) ip += offset; break;

        // JB, JNB
        case 0x82: offset = (int16_t)fetch_word(); if   (flags&C_FLAG)  ip += offset; break;
        case 0x83: offset = (int16_t)fetch_word(); if (!(flags&C_FLAG)) ip += offset; break;

        // JZ, JNZ
        case 0x84: offset = (int16_t)fetch_word(); if   (flags&Z_FLAG)  ip += offset; break;
        case 0x85: offset = (int16_t)fetch_word(); if (!(flags&Z_FLAG)) ip += offset; break;

        // JBE, JNBE
        case 0x86: offset = (int16_t)fetch_word(); if   (flags&(C_FLAG|Z_FLAG))  ip += offset; break;
        case 0x87: offset = (int16_t)fetch_word(); if (!(flags&(C_FLAG|Z_FLAG))) ip += offset; break;

        // JS, JNS
        case 0x88: offset = (int16_t)fetch_word(); if   (flags&N_FLAG)  ip += offset; break;
        case 0x89: offset = (int16_t)fetch_word(); if (!(flags&N_FLAG)) ip += offset; break;

        // JP, JNP
        case 0x8A: offset = (int16_t)fetch_word(); if   (flags&P_FLAG)  ip += offset; break;
        case 0x8B: offset = (int16_t)fetch_word(); if (!(flags&P_FLAG)) ip += offset; break;

        // JL, JNL
        case 0x8C: offset = (int16_t)fetch_word(); if (!!(flags&N_FLAG) != !!(flags&V_FLAG)) ip += offset; break;
        case 0x8D: offset = (int16_t)fetch_word(); if (!!(flags&N_FLAG) == !!(flags&V_FLAG)) ip += offset; break;

        // JLE, JNLE: ZF=1 OR (SF!=OF); ZF=0 AND (SF=OF)
        case 0x8E: offset = (int16_t)fetch_word(); if ( (flags&Z_FLAG) ||  !!(flags&N_FLAG) != !!(flags&V_FLAG))  ip += offset; break;
        case 0x8F: offset = (int16_t)fetch_word(); if (!(flags&Z_FLAG) && (!!(flags&N_FLAG) == !!(flags&V_FLAG))) ip += offset; break;
    ...
    while (instr_cnt-- >= 0) {
        ...
        if (segs[SEG_CS] == 0x7B0) {
            ...
            switch (opcode) {
                ...
                // JO, JNO
                case 0x70: offset = (int8_t)fetch_byte(); if   (flags&V_FLAG)  ip += offset; break;
                case 0x71: offset = (int8_t)fetch_byte(); if (!(flags&V_FLAG)) ip += offset; break;

                // JB, JNB
                case 0x72: offset = (int8_t)fetch_byte(); if   (flags&C_FLAG)  ip += offset; break;
                case 0x73: offset = (int8_t)fetch_byte(); if (!(flags&C_FLAG)) ip += offset; break;

                // JZ, JNZ
                case 0x74: offset = (int8_t)fetch_byte(); if   (flags&Z_FLAG)  ip += offset; break;
                case 0x75: offset = (int8_t)fetch_byte(); if (!(flags&Z_FLAG)) ip += offset; break;

                // JBE, JNBE
                case 0x76: offset = (int8_t)fetch_byte(); if   (flags&(C_FLAG|Z_FLAG))  ip += offset; break;
                case 0x77: offset = (int8_t)fetch_byte(); if (!(flags&(C_FLAG|Z_FLAG))) ip += offset; break;

                // JS, JNS
                case 0x78: offset = (int8_t)fetch_byte(); if   (flags&N_FLAG)  ip += offset; break;
                case 0x79: offset = (int8_t)fetch_byte(); if (!(flags&N_FLAG)) ip += offset; break;

                // JP, JNP
                case 0x7A: offset = (int8_t)fetch_byte(); if   (flags&P_FLAG)  ip += offset; break;
                case 0x7B: offset = (int8_t)fetch_byte(); if (!(flags&P_FLAG)) ip += offset; break;

                // JL, JNL
                case 0x7C: offset = (int8_t)fetch_byte(); if (!!(flags&N_FLAG) != !!(flags&V_FLAG)) ip += offset; break;
                case 0x7D: offset = (int8_t)fetch_byte(); if (!!(flags&N_FLAG) == !!(flags&V_FLAG)) ip += offset; break;

                // JLE, JNLE: ZF=1 OR (SF!=OF); ZF=0 AND (SF=OF)
                case 0x7E: offset = (int8_t)fetch_byte(); if ( (flags&Z_FLAG) ||  !!(flags&N_FLAG) != !!(flags&V_FLAG))  ip += offset; break;
                case 0x7F: offset = (int8_t)fetch_byte(); if (!(flags&Z_FLAG) && (!!(flags&N_FLAG) == !!(flags&V_FLAG))) ip += offset; break;
                ...
                case 0xA1: tempw = fetch_word(); AX_ = readmemw(segment, tempw); break;
                case 0xA2: tempw = fetch_word(); writememb(segment + tempw, AX_); break;
                case 0xA3: tempw = fetch_word(); writememw(segment, tempw, AX_); break;

                // MOVSB: Переместить байт
                case 0xA4: {

                    REPINIT;
                    writememb(seg_es + DI_, machine_read(segment + SI_));
                    REPINC(SI_,1);
                    REPINC(DI_,1);
                    autorep(0);
                    break;
                ...
                // LOOP[NZ|Z] JCXZ
                case 0xE0: offset = (int8_t) fetch_byte(); CX_--; if ( CX_ && !(flags & Z_FLAG)) ip += offset; break;
                case 0xE1: offset = (int8_t) fetch_byte(); CX_--; if ( CX_ &&  (flags & Z_FLAG)) ip += offset; break;
                case 0xE2: offset = (int8_t) fetch_byte(); CX_--; if ( CX_) ip += offset; break;
                case 0xE3: offset = (int8_t) fetch_byte();        if (!CX_) ip += offset; break;

                // IN/OUT #8
                case 0xE4: tempb = fetch_byte(); setr8(REG_AL, ioread(tempb)); break;
                case 0xE5: tempb = fetch_byte(); setr8(REG_AL, ioread(tempb)); setr8(REG_AH, ioread(tempb+1)); break;
                case 0xE6: tempb = fetch_byte(); iowrite(tempb, AL_); break;
                case 0xE7: tempb = fetch_byte(); iowrite(tempb, AL_); iowrite(tempb+1, AH_); break;
