...
#define RAM_SIZE       0x10FFF0     // 1mb + 64kb
#define REGS_BASE      0xF0000      // Регистры находятся в BIOS
...
#define DAA_DAS(op1,op2,mask,min) \
    set_AF((((scratch2_uint = regs8[REG_AL]) & 0x0F) > 9) || regs8[FLAG_AF]) && (op_result = regs8[REG_AL] op1 6, set_CF(regs8[FLAG_CF] || (regs8[REG_AL] op2 scratch2_uint))), \
...
#define R_M_OP(dest, op, src) \
    (i_w ? op_dest = CAST(unsigned short)dest, \
...
#define SDL_KEYBOARD_DRIVER sdl_screen ? \
    ...
    (scratch_uint = sdl_event.key.keysym.unicode, \
     scratch2_uint = sdl_event.key.keysym.mod, \
        ...
        0x800*!!(scratch2_uint & KMOD_ALT) + \
        0x1000*!!(scratch2_uint & KMOD_SHIFT) + \
        0x2000*!!(scratch2_uint & KMOD_CTRL) + \
        0x4000*(sdl_event.type == SDL_KEYUP) + \
            ((!scratch_uint || scratch_uint > 0x7F) ? sdl_event.key.keysym.sym : scratch_uint), \
        pc_interrupt(7)\
    ) : (KEYBOARD_DRIVER)
...
unsigned char
    mem[RAM_SIZE],
    io_ports[IO_PORT_COUNT],
    *opcode_stream,
    *regs8,                 // =F000:0000
    i_rm,
    i_w,                    // WIDE=opcode[0] byte|word
    i_reg,
    i_mod,
    i_mod_size,
    i_d,                    // DIR=opcode[1] rm,r | r,rm
    i_reg4bit,
    raw_opcode_id,
    xlat_opcode_id,
    extra,
...
SDL_Event sdl_event;
unsigned short
    vid_addr_lookup[VIDEO_RAM_SIZE],
    cga_colors[4] = {
        0x0000 /* Black */,
        0x1F1F /* Cyan */,
        0xE3E3 /* Magenta */,
...
{
    for (int i = 0; i < len; i++) {
        stream[i] = (spkr_en == 3) && CAST(unsigned short)mem[0x4AA] ?
...
int main(int argc, char **argv)
{
    ...
    for (file_index = 3; file_index;) disk[--file_index] = *++argv ? open(*argv, 32898) : 0;
    ...
    // opcode_stream != mem (пока указатель opcode_stream не равен указателю mem)
        ...
        switch (xlat_opcode_id)
        {
            ...
            case 12:
                ...
                scratch_uint = extra ? // xxx reg/mem, imm
                    ++reg_ip,
                    (char)i_data1 : // xxx reg/mem, CL
                    i_d ? 31 & regs8[REG_CL] : 1; // xxx reg/mem, 1
